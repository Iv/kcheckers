    ...
    : QFrame(parent)
{
    ...
    hb->addWidget(sq,  2);
    hb->addSpacing(5);
    m_history->setMinimumWidth(220);
    hb->addWidget(m_history, 1, Qt::AlignTop);
    ...
    m_log->resize(m_log->size().width(), 50);
    QVBoxLayout* vb = new QVBoxLayout(this);
    vb->addLayout(hb,  3);
    vb->addWidget(m_log, 1);
...
void MainView::setEnabled(bool b)
{
...
void MainView::setTheme(const QString& path)
{
...
void MainView::newGame(int rules, bool freeplace,
...
void MainView::begin_game(unsigned int round, bool freeplace)
{
    if(m_clear_log)
        m_log->clear();
...
bool MainView::check_game_over()
{
    ...
    if(!player_can && !opp_can) {
        add_log(MainView::System, tr("Drawn game."));
...
void MainView::slot_click(int field_num)
{
    ...
    if(m_history->isPaused()) {
        ...
        if(!m_current->fieldClicked(field_num, &select, err_msg)) {
            add_log(MainView::Warning, m_current->name()+": "
                    + (err_msg.length()
                       ? err_msg
                       : tr("Invalid move.")));
        } else {
...
void MainView::stop_game(const QString& msg)
{
    m_game_over = true;
    m_aborted = true;

    QString text(tr("Game aborted.")+(!msg.isEmpty() ? "\n"+msg : ""));
    add_log(MainView::System, text);

    emit working(false);
}


void MainView::slot_move_done(const QString& board_str)
{
...
void MainView::slot_move_done_step_two()
{
    //
...
bool MainView::openPdn(const QString& fn)
{
    emit working(false);
    ...
    if(log_text.length()) {
        add_log(MainView::System, tr("Opened:")+" "+fn);
        add_log(MainView::Error, log_text.trimmed());
        add_log(MainView::Warning, tr("Warning! Some errors occured."));
    }
...
bool MainView::savePdn(const QString& fn)
{
    if(!m_history->savePdn(fn)) {
        qDebug() << __PRETTY_FUNCTION__ << "failed.";
        return false;
    }
    add_log(MainView::System, tr("Saved:")+" "+fn);
    return true;
}


void MainView::slot_new_mode(bool paused, bool freeplace)
{
    if(paused) {
        if(freeplace)
...
void MainView::slot_preview_game(int rules)
{
...
void MainView::slot_apply_moves(const QString& moves)
{
...
void MainView::add_log(enum LogType type, const QString& text)
{
    QString str = text;
    str = str.replace('<', "&lt;");
    str = str.replace('>', "&gt;");
...
void MainView::perform_jumps(const QString& from_board, const QString& to_board)
{
...
void MainView::setNotation(bool enabled, bool show_above)
{
...
void MainView::setNotationFont(const QFont& f)
{
    // TODO - intermediate function - remove somehow!
...
myPlayer* MainView::get_first_player() const
{
